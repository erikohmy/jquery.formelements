...
}(function ($)
{
    ...
    $.tiny.scrollbar = {
        options: {
            ...
            ,   events       : {      // Allow the firing of events
                    scrolled : false, // triggers when the content is scrolled, either by drag or wheel
                    drag     : false, // triggers when using the handle
                    wheel    : false  // triggers when using the mousewheel
                }
        }
    };
    ...
    function Scrollbar($container, options)
    {
        ...
        function wheel(event)
        {
            if(contentRatio < 1)
            {
                ...
                var data = {
                    ...
                    bottom          : contentSize - viewportSize === contentPosition
                }; 

                // added hooks
                if( typeof( options.events.scrolled ) == 'function') {
                	options.events.scrolled( data );
                }
                if( typeof( options.events.wheel ) == 'function') {
                	options.events.wheel( data );
                }
            }
        }
        ...
        function drag( event )
        {
            if(contentRatio < 1)
            {
                ...
                if( typeof( options.events.drag ) == 'function') {
                	options.events.drag( data );
                }
            }
        }
